    ...
    <div className={`${status.bg} border ${status.border} rounded-xl p-5 transition-all duration-200 hover:scale-[1.01]`}>
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right shrink-0">
          ...
          {airdrop.claimUrl && airdrop.status === 'claimable' && (
            <a
...
export default function AirdropResults({ results, walletAddress }) {
  const totalValue = results.airdrops?.reduce((sum, a) => sum + (parseFloat(a.value) || 0), 0) || 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-5 text-center">
          ...
          <p className="text-2xl font-bold text-indigo-400">${totalValue.toLocaleString('en-US', { maximumFractionDigits: 2 })}</p>
      ...
      {results.airdrops && results.airdrops.length > 0 ? (
        ...
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-8 text-center">
          <p className="text-[var(--color-text-muted)] text-lg">Tidak ada airdrop yang ditemukan untuk wallet ini</p>
          <p className="text-[var(--color-text-muted)] text-sm mt-2">Coba gunakan alamat wallet yang berbeda atau periksa jaringan lain</p>
        </div>
      )}

      {/* Link to Drops.bot */}
      <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-5 text-center">
        <p className="text-[var(--color-text-muted)] text-sm mb-3">Lihat detail lengkap & klaim langsung di Drops.bot</p>
        <a
          href={`https://drops.bot/address/${walletAddress}`}
          target="_blank"
          rel="noopener noreferrer"
